...
const FEATURED_SCENARIO: DemoScenario = {
  ...
  label: 'Work restart',
  title: 'งานค้าง แล้วต้องหาก้าวแรกใหม่',
  detail: 'เหมาะกับตอนที่งานหยุดไปหลายวันและต้องกลับเข้า context ให้เร็ว โดยไม่ต้องอ่านทุกอย่างใหม่ตั้งแต่ต้น',
  value: 'งานค้างมาสองสัปดาห์ มี notes กับข้อความเก่าอยู่หลายที่ ตอนนี้อยากรู้ว่างานอยู่ตรงไหนแล้ว และควรเริ่มจากอะไรให้กลับเข้าร่องได้เร็วที่สุด',
  ...
  const copy = entryVariant === 'get_started'
    ...
    : entryVariant === 'active_context'
      ? {
          ...
          placeholder: 'เพิ่มข้อมูลใหม่ของงานนี้ เช่น มีข้อความใหม่ / scope เปลี่ยน / มีไฟล์หรือ note เพิ่ม...',
      ...
      : {
          ...
          placeholder: 'พิมพ์สภาพงานตอนนี้ตรง ๆ ได้เลย เช่น notes กระจัดกระจาย / งานค้างไปหลายวัน / ยังไม่รู้จะเริ่มตรงไหน...',
          submitLabel: 'ไปต่อ',
          submittingLabel: 'กำลังสรุป...',
          showExamples: true,
        };
          ...
          >
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
              ...
              <span style={{ color: 'var(--text-secondary)', fontSize: '0.84rem', lineHeight: 1.55 }}>
                ข้อความและไฟล์เดิมยังอยู่ ลองปรับให้ตรงเคสนี้มากขึ้น แล้วกด ‘เพิ่มบริบท’ อีกครั้ง
              </span>
            </div>
          </div>
        )}
