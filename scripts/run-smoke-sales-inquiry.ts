...
const DEFAULT_PORT = Number(process.env.MIND_SALES_INQUIRY_PORT || 3205) || 3205;
...
async function runBrowserSmoke(baseUrl: string) {
  return new Promise<number>((resolve, reject) => {
    const child = spawn('tsx', ['scripts/browser-verify-sales-inquiry.ts'], {
      cwd: process.cwd(),
      env: {
        ...process.env,
...
async function main() {
  ...
  console.log(JSON.stringify({ smoke: 'sales-inquiry', baseUrl, source: 'fresh-prod-server' }, null, 2));
...
main().catch((error) => {
  console.error('[SMOKE] sales inquiry wrapper failed', error);
  process.exitCode = 1;
});
